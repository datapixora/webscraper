...
db-shell: ## Access PostgreSQL shell
	docker compose exec db psql -U webscraper -d webscraper_db
...
backup-db: ## Backup database
	...
	docker compose exec -T db pg_dump -U webscraper webscraper_db > backup_$(shell date +%Y%m%d_%H%M%S).sql
	@echo "$(GREEN)Database backup created$(NC)"

restore-db: ## Restore database (usage: make restore-db FILE=backup.sql)
	@echo "$(BLUE)Restoring database from $(FILE)...$(NC)"
	docker compose exec -T db psql -U webscraper -d webscraper_db < $(FILE)
